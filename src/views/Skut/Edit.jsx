...
class Edit extends Component {
    ...
    componentDidMount() {
        ...
        if (this.props.match.params.id) {
            let comp_this = this;
            let skut_id = this.props.match.params.id;
            axios.get(config.auth.apiBaseUrl + "/api/scooters/" + skut_id + "/")
                .then(function (response) {

                    if (response.data) {
                        let selectedSkutModel = {
                            value: response.data.model.id,
                            label: response.data.model.name
                        };
                        let selectedLockModel = {
                            value: response.data.lock.id,
                            label: response.data.lock.name
                        } ;
                        let selectedStandModel = {
                            value: response.data.stand.id,
                            label: response.data.stand.name
                        };
                        let keyName = response.data.key_name;
                        let description = response.data.description;
                        let id = response.data.id;
                        comp_this.setState({selectedSkutModel, selectedLockModel, selectedStandModel, keyName, description, id});
    ...
    onSkutSubmit = (e) => {
        ...
        axios.patch(config.auth.apiBaseUrl + "/api/scooters/"+ id +"/", params)
            .then(function (response) {
                            ...
                            <div className="form-login">
                                ...
                                <Col lg={6} sm={6} >
                                    ...
                                    <FormGroup
                                        controlId="model">
                                        <Select
                                            value={this.state.selectedSkutModel}
